...
public interface PatientRepository extends CrudRepository<Patient, Integer> {
    ...
    @Modifying
    @Query(
        value = "INSERT INTO user_roles (doctor_id,role_id,patient_id) VALUES (NULL,:roleId,:userId);",
        nativeQuery = true
    )
    public void addRole(@Param("userId") int userId, @Param("roleId") long roleId);
        ...
        @Query(
                value = "SELECT * FROM patient WHERE patient.user_first_name LIKE ?1 OR patient.user_last_name LIKE ?1",
                nativeQuery = true
        )
        List<Patient>searchByFirstNameAndLastName(String name);
}
